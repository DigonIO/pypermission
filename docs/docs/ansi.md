...
| `add_ascendant(asc: Name, desc: Role)`       | creates asc **Role** and its relation to desc | _N/A_                                     |
| `add_descendant(asc: Role, desc: Name)`      | creates desc **Role** and its relation to asc | _N/A_                                     |

We decided against implementing a `add_ascendant` or `add_descendant` function, as the we believe that the suggested behaviour is ambiguous when taking a glance at the function name.
