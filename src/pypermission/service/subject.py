...
def create(*, subject: str, db: Session) -> None:
    try:
...
def delete(*, subject: str, db: Session) -> None:
...
def assign_role(*, subject: str, role: str, db: Session) -> None:
    try:
...
def deassign_role(*, subject: str, role: str, db: Session) -> None:
...
def assigned_roles(*, subject: str, db: Session) -> tuple[str, ...]:
    roles = db.scalars(
...
def assert_permission(
    *,
    ...
    if not check_permission(subject=subject, permission=permission, db=db):
        raise PyPermissionNotGrantedError()


def policies(*, subject: str, db: Session) -> tuple[Policy, ...]:
    root_cte = (
        select(MemberORM.role_id)
        .where(MemberORM.subject_id == subject)
        .cte(recursive=True)
