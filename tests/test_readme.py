...
def test_readme_code_example():
    ...
    assert auth.subject_obtains_permission(sid="Bob", node=Nodes.CHAT_GLOBAL)
    assert auth.subject_obtains_permission(sid="Alice", node=Nodes.CHAT_MODERATOR)
    assert auth.subject_obtains_permission(sid="Bob", node=Nodes.TICKET_OPEN)
    assert auth.subject_obtains_permission(sid="Alice", node=Nodes.TICKET_CLOSE_ALL)
