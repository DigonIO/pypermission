...
def test_rm_permission(serial_authority: SerialAuthority):
    ...
    assert auth.role_obtains_permission(rid=FOOD, node=TPN.TOWNY_CHAT_GLOBAL) == True
    auth.role_revoke_permission(rid=FOOD, node=TPN.TOWNY_CHAT_GLOBAL)
    assert auth.role_obtains_permission(rid=FOOD, node=TPN.TOWNY_CHAT_GLOBAL) == False

    assert auth.subject_obtains_permission(sid=HAM, node=TPN.TOWNY_WILD_) == True
    auth.subject_rm_node(sid=HAM, node=TPN.TOWNY_WILD_)
    assert auth.subject_obtains_permission(sid=HAM, node=TPN.TOWNY_WILD_) == False
